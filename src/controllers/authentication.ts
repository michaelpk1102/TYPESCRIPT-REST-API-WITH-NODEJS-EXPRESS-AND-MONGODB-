...
import { createUser, getEmail } from "../db/user";
 ...
 export const login = async (req: express.Request, res:express.Response) => {
    ...
    const user = await getEmail(email).select("+authentication.salt +authentication.password")
    if(!user){
        return res.sendStatus(400)
    }
    ...
    res.cookie("EMMANUEL-AUTH", user.authentication.sessionToken, {domain:"localhost",path: "/"})
